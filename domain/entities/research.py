...
class ResearchProgressType(str, Enum):
    """Enum for research progress types"""
...
class ResearchProgress(BaseModel):
    """Progress update model for streaming research operations"""
    type: ResearchProgressType = Field(description="The type of progress event")
    task_id: str = Field(description="Unique identifier for the specific task")
    job_id: str = Field(description="Unique identifier for the research job")
    message: str = Field(description="Human-readable progress message (concise one liner)")
    progress: Optional[float] = Field(default=None, description="Progress percentage (0.0 to 1.0)")
    data: Optional[Dict[str, Any]] = Field(default=None, description="Additional data assoc iated with the progress event")